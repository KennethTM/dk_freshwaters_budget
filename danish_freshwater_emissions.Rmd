...
upscale_total <- bind_rows(bind_rows(add_column(so_upscale_sizecat, system = "lake"), 
  ...
  add_row(mean = vl_upscale_total$mean + so_upscale_total$mean,
          q_50 = vl_upscale_total$q_50 + so_upscale_total$q_50,
          sizecat = "grand total") 
...
lab_expressions <- rev(c(expression(0-2.5~m), expression(2.5-12~m), expression(">12"~m),
                     expression(0-10^3~m^2), expression(10^3-10^4~m^2), 
                     expression(10^4-10^5~m^2), expression(10^5-10^6~m^2),
                     expression(10^6-10^7~m^2), expression(10^7-10^8~m^2)))
...
ridge_pressure <- so_vl_flux_rates %>% 
  ...
  geom_vline(xintercept = 410, linetype = 1, col = "black", size = 1)+
...
ggsave("ridge_pressure_flux.png", ridge_pressure/ridge_flux, width = 200, height = 200, units = "mm")
...
alk <- ggplot(grw_df$alk, aes(x, y, fill = layer))+
  geom_raster()+
  scale_fill_viridis_c(name = expression("Alkalinitet (meq "*L^{-1}*")"), na.value = "white", breaks = seq(0, 10, 2), direction = -1)+
  theme_void()+
  coord_equal()+
...
ph <- ggplot(grw_df$ph, aes(x, y, fill = layer))+
  geom_raster()+
  scale_fill_viridis_c(name = "pH", na.value = "white", breaks = seq(4, 8, 0.5), direction = -1)+
  theme_void()+
  coord_equal()+
  theme(legend.position = c(0.8, 0.7), legend.justification = "left")
...
pco2 <- grw_df$pco2 %>% 
  ...
  na.omit() %>% 
  ggplot(aes(x, y, fill = bin))+
  geom_raster()+
  #scale_fill_viridis_c(name = "pH", na.value = "white", option = "C", breaks = seq(0, 40000, 10000), direction = -1)+
